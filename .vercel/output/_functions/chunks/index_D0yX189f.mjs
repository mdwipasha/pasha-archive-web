...
import 'piccolore';
import { o as renderComponent, k as renderTemplate, m as maybeRenderHead } from './entrypoint_dDfbRc3X.mjs';
...
function GallerySection({ memories }) {
  ...
  const totalActiveFilters = selectedTags.length + selectedTypes.length + selectedYears.length + selectedLocations.length + selectedPeople.length;
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const peopleById = new Map(
    peopleList.map((person) => [
      person.id,
      { name: person.name, social_media: person.social_media }
    ])
  );
...
A collection of moments captured across time — and
                        memories worth keeping.
</p> </div> </div> ${renderComponent($$result2, "GallerySection", GallerySection, { "client:load": true, "memories": memories, "client:component-hydration": "load", "client:component-path": "D:/CODING/pasha-archive-web/src/components/Gallery/GallerySection.jsx", "client:component-export": "default" })} </div> </main> ${renderComponent($$result2, "Footer", $$Footer, {})} ` })}`;
}, "D:/CODING/pasha-archive-web/src/pages/galleries/index.astro", void 0);

const $$file = "D:/CODING/pasha-archive-web/src/pages/galleries/index.astro";
const $$url = "/galleries";
