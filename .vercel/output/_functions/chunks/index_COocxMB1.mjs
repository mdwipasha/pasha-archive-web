...
import 'piccolore';
import { p as maybeRenderHead, u as renderTemplate, j as addAttribute, q as renderComponent } from './entrypoint_BVRuuQGF.mjs';
...
const $$RecentArchive = createComponent(async ($$result, $$props, $$slots) => {
  const { data: featuredMemories } = await supabase.from("memories").select("*").eq("featured", true).order("created_at", { ascending: false }).limit(3);
